...
!developmentChains.includes(network.name)
    ? describe.skip// unit test performing only for localhost/hardhat network
    : describe("Lottery Unit Tests", function () {
        let lottery, vrfCoordinatorV2Mock, lotteryContract, lotteryMinContribution, interval, player, deployer
        beforeEach(async () => {
            ...
            vrfCoordinatorV2Mock = await ethers.getContract("VRFCoordinatorV2Mock") // Returns a new connection to the VRFCoordinatorV2Mock contract and defaulty connected to the deployer account
            ...
            lotteryMinContribution = await lottery.getMinContribution()
        ...
        describe("constructor", function () {
            it("initializes the lottery correctly", async () => {
                ...
                //we have to write separate it blocks for all the args passed to constructor
            })
        })
        describe("enterLottery", function () {
            it("reverts when you don't pay enough", async () => {
                await expect(lottery.enterLottery()).to.be.revertedWith( // is reverted when not paid enough or lottery is not open
                    "Lottery__SendMoreToEnterLottery"
                )
            })
            it("records player when they enter", async () => {
                await lottery.enterLottery({ value: lotteryMinContribution })
                const contractPlayer = await lottery.getPlayer(0)
                assert.equal(player.address, contractPlayer)
            })
            it("emits event on enter", async () => {
                await expect(lottery.enterLottery({ value: lotteryMinContribution })).to.emit( // emits LotteryEnter event if entered to index player(s) address
                    lottery,
                    "LotteryEnter"
                )
            })
            it("doesn't allow entrance when lottery is calculating", async () => {
                ...
                await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])//to increase the time on blockchain
                await network.provider.request({ method: "evm_mine", params: [] })//block must be mined after increasing the time 
                // we pretend to be a keeper for a second becoz keeper call he performUpKeep
                await lottery.performUpkeep([]) // changes the state to calculating for our comparison below
                await expect(lottery.enterLottery({ value: lotteryMinContribution })).to.be.revertedWith( // is reverted as lottery is calculating
        ...
        describe("checkUpkeep", function () {
            it("returns false if people haven't sent any ETH", async () => {
                ...
                //as checkUpkeep is a public function (not public view) so it is trying
                //to send a transaction but here we just want to simulate the transaction 
                //so we use callStatic
                assert(!upkeepNeeded)
            })
                        ...
                        try {
                            ...
                            assert.equal(
                                ...
                                startingBalance // startingBalance + ( (raffleEntranceFee * additionalEntrances) + raffleEntranceFee )
                                    .add(
                                        lotteryMinContribution
                                            .mul(additionalEntrances)
                                            .add(lotteryMinContribution)
                                    )
                                    .toString()
                            )
